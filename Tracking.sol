...
contract Tracking {
    ...
    function createParcel(address customerAddress, uint role_id) public onlyCourierOrAdmin(role_id){
        parcelId++;
        parcel[parcelId] = parcelInfo(customerAddress, 0);
        emit Status(parcelId, "Parcel added to blockchain.", "N/A");
    }
    ...
    function updateStatus (uint _parcelId, string memory report, string memory imageURL, uint role_id) public onlyCourierOrAdmin (role_id){
        ...
        emit Status(parcelId, report, imageURL);
    }
